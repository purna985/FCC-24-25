...
const items = [
  {
    id: 1,
    image: 'https://placehold.co/600x400/0A0A0A/fff?text=Event+1',
    date: 'Mar 2024',
    title: 'Horizon 2024',
  ...
  {
    id: 2,
    image: 'https://placehold.co/600x400/1A1A1A/fff?text=Event+2',
    date: 'Mar 2024',
    title: 'Horizon 2024',
  ...
  {
    id: 3,
    image: 'https://placehold.co/600x400/2A2A2A/fff?text=Event+3',
    date: 'Mar 2024',
    title: 'Horizon 2024',
  ...
  {
    id: 4,
    image: 'https://placehold.co/600x400/3A3A3A/fff?text=Event+4',
    date: 'Mar 2024',
    title: 'Horizon 2024',
    description: 'Donec venenatis arcu eu nisi. Vestibulum eu ante et elit accumsan. Sed vel lectus. Ut varius tincidunt sem. Proin vulputate arcu sit amet nunc.',
    attendees: '250 Attendees',
    location: 'Auditorium'
  },
  {
    id: 5,
    image: 'https://placehold.co/600x400/4A4A4A/fff?text=Event+5',
    date: 'Mar 2024',
    title: 'Horizon 2024',
    description: 'Aliquam eu nunc. Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Nam leo.',
    attendees: '250 Attendees',
    location: 'Auditorium'
  },
];
...
const HomeCarousel = () => {
  ...
  // We repeat the items three times: [original, duplicate_1, duplicate_2].
  // This allows for seamless transitions when looping back.
  const duplicatedItems = [...items, ...items, ...items];

  // useEffect to calculate the width of a single carousel item including its gap.
  // This value is crucial for correctly translating the carousel.
  useEffect(() => {
    const calculateItemDimensions = () => {
      if (itemRef.current && carouselTrackRef.current) {
        ...
        // Initialize `currentIndex` to the beginning of the *second* set of items (`items.length`).
        // This setup allows for smooth forward scrolling into the second set and seamless
        // backward looping from the first item of the first set to the last item of the third set.
        // We ensure this only happens once when the component mounts and `itemWidth` is first determined.
  ...
  const CarouselCard = ({ item, innerRef }) => (
    <div
      ...
      // `flex-shrink-0` prevents items from shrinking. `w-80` sets a fixed width (320px).
      // Styling applied with Tailwind CSS classes to match the provided image.
      className="flex-shrink-0 w-80 bg-gray-800 rounded-xl shadow-lg p-4 border border-gray-700 font-inter"
    >
      {/* Image section */}
      ...
      <div className="flex flex-wrap gap-2 justify-start">
        <button className="flex items-center px-4 py-2 bg-indigo-700 text-white text-sm rounded-full shadow-md hover:bg-indigo-600 transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-opacity-50">
          {/* Attendees icon (SVG) */}
  ...
  return (
    <div className="min-h-screen bg-gray-900 flex items-center justify-center py-10 px-4 sm:px-6 lg:px-8 font-inter">
